...
import './DanhSachSVGuiDangKyTT.css';
...
function DanhSachSVDuocXacNhanTuCLB() {
  ...
  const isCurrentlyRejected = item.tinhTrangXacNhan === 'Bị từ chối';
  const newStatus = isCurrentlyRejected ? 'Không' : 'Bị từ chối'; // Trạng thái rõ ràng, không dùng null

  try {
    await axios.put(apiUpsertGV, {
      mssv: item.mssv,
      maDotThucTap: item.maDotThucTap,
      tinhTrangXacNhan: newStatus
    });

    updateTinhTrang(item.mssv, item.maDotThucTap, newStatus);
  } catch (e) {
